...
if result.is_error():
    ...
    announcements = sdk.getAnnouncements()
    if announcements.is_error():
        if announcements.get_wilma_error() is not None:
            print(announcements.get_wilma_error()['message'])
            print("--> " + announcements.get_wilma_error()['description'])
        else:
            print(announcements.get_exception())
    else:
        print("Got announcements")
        for announcement in announcements.announcements:

            print(" "+str(announcement['id'])+" "+announcement['topic'])
            print("By "+announcement['creator'])
            print(announcement)
