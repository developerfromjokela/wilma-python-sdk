...
if result.is_error():
    ...
    num = input("Enter Announcement ID: \n")
    numInt = int(num)
    announcement = sdk.getAnnouncement(numInt)
    if announcement.is_error():
        if announcement.get_wilma_error() is not None:
            print(announcement.get_wilma_error()['message'])
            print("--> " + announcement.get_wilma_error()['description'])
        else:
            print(announcement.get_exception())
    else:

        print("--> " + str(announcement.announcement['id']) + " " + announcement.announcement['topic'])
        print("By " + announcement.announcement['creator'])
        print(announcement.announcement['created'])
        print("-----")
        print("Content of announcement:")
        print(announcement.announcement['content']['text'])
        print("HTML:")
        print(announcement.announcement['content']['html'])
